...
__export(index_exports, {
  ...
  getIndustrialSession: () => getIndustrialSession,
  getTenantSubdomain: () => getTenantSubdomain,
  verifyToken: () => verifyToken,
  withIndustrialAuth: () => withIndustrialAuth
});
...
async function verifyToken(token, customSecret) {
  try {
    const { payload } = await (0, import_jose.jwtVerify)(token, getSecretKey(customSecret));
    return payload;
  } catch (err) {
    return null;
  }
...
function withIndustrialAuth(options) {
  ...
  return async function middleware(request) {
    ...
    if (isAsset) {
      return options.intlMiddleware ? options.intlMiddleware(request) : import_server.NextResponse.next();
    }
    ...
    if (subdomain) {
      ...
      if (!tenantInfo || !tenantInfo.active) {
        ...
        return import_server.NextResponse.redirect(new URL(`${baseAppUrl}/logout-success?error=tenant_not_found`));
      }
    }
    ...
    if (!isAuthenticated) {
      ...
      const response2 = import_server.NextResponse.redirect(authorizeUrl);
    ...
    const response = options.intlMiddleware ? await options.intlMiddleware(request) : import_server.NextResponse.next();
...
var import_css_generator = require("@abd/styles/dist/engine/css-generator.js");
...
async function BrandingStyles({
  ...
  try {
    ...
    const customCss = branding?.theme ? (0, import_css_generator.generateTenantCss)(branding.theme) : null;
    const faviconUrl = branding?.favicon?.url || null;
    if (!customCss && !faviconUrl) {
      return null;
    }
    return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
      customCss && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        "style",
        {
      ...
      faviconUrl && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("link", { rel: "icon", href: faviconUrl })
    ] });
  } catch (err) {
...
function createAuthRouteHandler(options) {
  ...
  return async function handler(request) {
    ...
    if (pathname.endsWith("/session")) {
      ...
      return import_server2.NextResponse.json(session);
    }
    if (pathname.endsWith("/logout")) {
      ...
      if (isSilent) {
        const response2 = new import_server2.NextResponse(null, { status: 200 });
      ...
      const response = import_server2.NextResponse.redirect(
    ...
    if (pathname.endsWith("/federated/callback")) {
      ...
      if (!code) {
        return import_server2.NextResponse.json({ error: "No authorization code provided" }, { status: 400 });
      }
      try {
        ...
        if (!res.ok) {
          ...
          return import_server2.NextResponse.json({ error: "Token exchange failed", detail: errorData }, { status: 401 });
        }
        const data = await res.json();
        const redirectResponse = import_server2.NextResponse.redirect(new URL(state, request.url));
        ...
        return import_server2.NextResponse.json({ error: "Internal server error during token exchange" }, { status: 500 });
      }
    }
    return import_server2.NextResponse.json({ error: "Route not found" }, { status: 404 });
  };
}
